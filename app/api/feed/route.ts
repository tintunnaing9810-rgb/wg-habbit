...
export async function GET() {
  ...
  // Get reactions for all these logs
  ...
  // Aggregate reactions per log
  const reactionMap = new Map<string, Map<string, number>>();
  for (const r of reactions ?? []) {
    if (!reactionMap.has(r.log_id)) {
      reactionMap.set(r.log_id, new Map());
    }
  ...
  const feedData = logs.map((log) => {
    ...
    return {
      ...
      user: {
        ...
        avatar_url: logUser?.avatar_url ?? null,
      },
