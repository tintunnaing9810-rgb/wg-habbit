...
export default function FeedPage() {
  ...
  return (
    <div className="px-4 pt-3 pb-6 space-y-4">
      ...
      {/* Discover tab — only shows people you don't follow yet */}
      {activeTab === "discover" && (() => {
        const unfollowed = discover.filter((u) => !u.is_following);
        return discoverLoading ? (
          ...
          <div className="card p-8 text-center">
            <p className="text-3xl mb-3">🎉</p>
            <p className="text-sm font-medium text-slate-700">You&apos;re following everyone!</p>
            <p className="text-xs text-slate-500 mt-1">Check the Following tab to see who you follow</p>
          </div>
        ) : (
          <div className="card divide-y divide-slate-100 overflow-hidden">
            {unfollowed.map((u) => (
              <div key={u.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <button
                  onClick={() => toggleFollowDiscover(u.id, false)}
                  className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold bg-indigo-600 text-white hover:bg-indigo-700 transition-all"
                >
